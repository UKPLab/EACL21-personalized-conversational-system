...
from pytorch_pretrained_bert import cached_path, CONFIG_NAME, WEIGHTS_NAME
...
PRETRAINED_CONFIG_ARCHIVE_MAP = {"openai-gpt": "https://s3.amazonaws.com/models.huggingface.co/bert/openai-gpt-bert_config.json"}
...
class OpenAIGPTDoubleHeadsModel_old(OpenAIGPTPreTrainedModel):
    ...
    def __init__(self, config):
        super(OpenAIGPTDoubleHeadsModel_old, self).__init__(config)
        self.transformer = OpenAIGPTModel(config)
        self.lm_head = OpenAIGPTLMHead(self.transformer.tokens_embed.weight, config)
    ...
    def forward(self, input_ids, mc_token_ids=None, lm_labels=None, mc_labels=None, token_type_ids=None, position_ids=None, weights=None, return_states=False):
        test_input_ids_arr = input_ids.cpu().detach().numpy()
        hidden_states = self.transformer(input_ids, position_ids, token_type_ids)
        lm_logits = self.lm_head(hidden_states)
        if mc_token_ids is not None:
            ...
            if mc_labels is not None:
                ...
                test_1 = mc_logits.view(-1, mc_logits.size(-1))
                test_2 = mc_labels.view(-1)
                losses.append(loss_fct(mc_logits.view(-1, mc_logits.size(-1)), mc_labels.view(-1)))
            if losses:
                return losses
            return lm_logits, mc_logits
        else:   # For RL part
            lm_outs = self.linear(hidden_states)
            lm_out = torch.mean(lm_outs.squeeze()).item()
            if lm_labels is not None:
                # Shift so that tokens < n predict n
                shift_logits = lm_logits[..., :-1, :].contiguous()
                shift_labels = lm_labels[..., 1:].contiguous()
                # Flatten the tokens
                loss_fct = CrossEntropyLoss(ignore_index=-1)
                test_1 = shift_logits.view(-1, shift_logits.size(-1))
                test_2 = shift_labels.view(-1)
                loss = loss_fct(shift_logits.view(-1, shift_logits.size(-1)),
                                shift_labels.view(-1))
...
class OpenAIGPTDoubleHeadsModel(OpenAIGPTPreTrainedModel):
    ...
    '''
    def forward(self, input_ids, mc_token_ids=None, lm_labels=None, mc_labels=None, token_type_ids=None, position_ids=None):
        hidden_states = self.transformer(input_ids, position_ids, token_type_ids)
        lm_logits = self.lm_head(hidden_states)
        if mc_token_ids is not None:
            mc_logits = self.multiple_choice_head(hidden_states, mc_token_ids)
        ...
        if mc_logits is not None:
            ...
            return lm_logits
            '''
    def forward(self, input_ids, mc_token_ids, lm_labels=None, mc_labels=None, token_type_ids=None, position_ids=None, gen=False,
                rl_train=False):
        if not rl_train:
            hidden_states = self.transformer(input_ids, position_ids, token_type_ids)
            lm_logits = self.lm_head(hidden_states)
